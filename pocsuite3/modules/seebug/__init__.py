...
class Seebug():
    ...
    def token_is_available(self):
        if self.token:
            ...
            try:
                resp = requests.get('https://www.disense.co/api/user/poc_list', headers=headers)
    ...
    def get_available_pocs(self):
        if self.check_account():
            try:
                resp = requests.get('https://www.disense.co/api/user/poc_list', headers=self.headers)
    ...
    def search_poc(self, keyword):
        if self.check_account():
            try:
                resp = requests.get('https://www.disense.co/api/user/poc_list?q=%s' % keyword, headers=self.headers)
                if resp and resp.status_code == 200:
    ...
    def fetch_poc(self, ssvid):
        if self.check_account():
            try:
                ...
                resp = requests.get('https://www.disense.co/api/user/poc_detail?id=%s' % ssvid, headers=self.headers)
